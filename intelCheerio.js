...
// (async () => {
    ...
    fs.writeFile(`intel.csv`, `team,entry,notes,wiki entry,aff,plan text,advantages,neg ,k,da,cp,t,theory,case turns,case answers,dumpMisc,,\n`, function (err) {
    ...
    for (j = 0; j < links.length; j++) {
        ...
        var $ = cheerio.load(affPageRes.text)
        ...
        if (metaThings.wikiEntryAffLink.includes("Aff")) { //its aff intel
            ...
            // affReturnData.aff = await page.evaluate(() => {
                var titles = []
                for (i = 1; i <= $('#tblCites').children('tbody').children('tr').length-1; i++) { // CHECK (ELEMENT POSITIONING)
                    // titles.push(document.querySelector(`#tblCites > tbody >tr:nth-child(${i}) > td:nth-child(1) > div > h4`).innerText)
                    titles.push($($($('#tblCites').children('tbody').children('tr')[i]).children('td')[0]).children('div').children('h4').children('span').text().trim())
                }
            ...
            if (affReturnData.aff === [] || (affReturnData.aff.length === 1 && affReturnData.aff[0].toLowerCase().includes('contact'))) {
                ...
                // affReturnData.aff = await page.evaluate(() => {
                    ...
                    for (i = 1; i < $('#tblReports').children('tbody').children('tr').length-1; i++) {
                        // eachReportArgs = document.querySelector("#tblReports > tbody >tr:nth-child(2) > td:nth-child(3) > div > div").innerText.split('\n')
                        eachReportArgs = $($($('#tblReports').children('tbody').children('tr')[1]).children('td')[2]).children('div').children('div').children('p').find('br').replaceWith('\n')
                        eachReportArgs = $($($('#tblReports').children('tbody').children('tr')[1]).children('td')[2]).children('div').children('div').children('p').find('p').replaceWith('\n')
                        eachReportArgs = eachReportArgs.text()
